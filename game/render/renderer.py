...
def _build_thorim_wireframe() -> list[tuple[Vector3, Vector3]]:
    ...
    def _direction_vector(angle: float) -> tuple[float, float]:
        direction_x = math.cos(angle) * outer_radius_x
        direction_z = math.sin(angle) * outer_radius_z
        length = math.hypot(direction_x, direction_z)
        if length == 0.0:
            return 0.0, 1.0
        return direction_x / length, direction_z / length

    def _perpendicular_vector(angle: float) -> tuple[float, float]:
        dir_x, dir_z = _direction_vector(angle)
        return -dir_z, dir_x

    def _nearest_by_z(points: Sequence[Vector3], target_z: float) -> Vector3:
        return min(points, key=lambda point: abs(point.z - target_z))
    ...
    # Dorsal and ventral structural spines running the length of the hull
    dorsal_spine_points = [
        Vector3(0.0, ring_height * 1.55, engine_z - size_scale * 0.3),
        Vector3(0.0, ring_height * 1.48, -outer_radius_z * 0.6),
        Vector3(0.0, ring_height * 1.38, -outer_radius_z * 0.2),
        Vector3(0.0, ring_height * 1.22, inner_radius_z * 0.18),
    ]
    ventral_spine_points = [
        Vector3(0.0, -ring_height * 1.55, engine_z - size_scale * 0.25),
        Vector3(0.0, -ring_height * 1.45, -outer_radius_z * 0.65),
        Vector3(0.0, -ring_height * 1.34, -outer_radius_z * 0.25),
        Vector3(0.0, -ring_height * 1.18, inner_radius_z * 0.12),
    ]

    for points in (dorsal_spine_points, ventral_spine_points):
        for idx in range(len(points) - 1):
            segments.append((points[idx], points[idx + 1]))

    for dorsal_point, ventral_point in zip(dorsal_spine_points, ventral_spine_points):
        segments.append((dorsal_point, ventral_point))

    # Connect ring sections into the spines to create visible ribbing without blocking the siege gap
    support_offsets = (8.0, 18.0, 32.0, 48.0)

    def _wrapped_angle(base_angle: float, delta_degrees: float) -> float:
        angle = base_angle + math.radians(delta_degrees)
        return angle % math.tau

    for delta in support_offsets:
        port_angle = _wrapped_angle(gap_center + gap_half_angle, delta)
        starboard_angle = _wrapped_angle(gap_center - gap_half_angle, -delta)

        for angle in (port_angle, starboard_angle):
            top_point = _ring_point(angle, outer_radius_x, outer_radius_z, ring_height * 0.92)
            middle_point = _ring_point(angle, inner_radius_x, inner_radius_z, 0.0)
            bottom_point = _ring_point(angle, outer_radius_x, outer_radius_z, -ring_height * 0.92)

            dorsal_anchor = _nearest_by_z(dorsal_spine_points, top_point.z)
            ventral_anchor = _nearest_by_z(ventral_spine_points, bottom_point.z)
            ...
            segments.extend(
                [
                    ...
                    (top_point, dorsal_anchor),
                    (bottom_point, ventral_anchor),
                ]
            )

            inner_top = _ring_point(angle, inner_radius_x, inner_radius_z, ring_height * 0.55)
            inner_bottom = _ring_point(angle, inner_radius_x, inner_radius_z, -ring_height * 0.55)
            segments.extend(
                [
                    (inner_top, middle_point),
                    (inner_bottom, middle_point),
                    (inner_top, dorsal_anchor),
                    (inner_bottom, ventral_anchor),
                ]
            )

    # Reinforce the aft profile with cross beams contained to the rear arc
    aft_cross_pairs = [
        (math.radians(205.0), math.radians(335.0)),
        (math.radians(225.0), math.radians(315.0)),
        (math.radians(250.0), math.radians(300.0)),
    ]

    for left_angle, right_angle in aft_cross_pairs:
        left_top = _ring_point(left_angle, outer_radius_x, outer_radius_z, ring_height * 0.88)
        left_bottom = _ring_point(left_angle, outer_radius_x, outer_radius_z, -ring_height * 0.88)
        right_top = _ring_point(right_angle, outer_radius_x, outer_radius_z, ring_height * 0.88)
        right_bottom = _ring_point(right_angle, outer_radius_x, outer_radius_z, -ring_height * 0.88)
        ...
        dorsal_anchor = _nearest_by_z(dorsal_spine_points, left_top.z)
        ventral_anchor = _nearest_by_z(ventral_spine_points, left_bottom.z)
        segments.extend(
            [
                (left_top, dorsal_anchor),
                (right_top, dorsal_anchor),
                (left_bottom, ventral_anchor),
                (right_bottom, ventral_anchor),
            ]
        )

    # Define the prominent siege arms that frame the front gap
    front_angles = (
        gap_center - gap_half_angle,
        gap_center + gap_half_angle,
    )
    arm_extension = size_scale * 1.85
    arm_lower_extension = arm_extension * 0.92
    arm_width = size_scale * 0.42

    for angle in front_angles:
        top_base = _ring_point(angle, outer_radius_x, outer_radius_z, ring_height * 0.9)
        bottom_base = _ring_point(angle, outer_radius_x, outer_radius_z, -ring_height * 0.9)
        middle_base = _ring_point(angle, inner_radius_x * 0.94, inner_radius_z * 0.94, 0.0)

        dir_x, dir_z = _direction_vector(angle)
        perp_x, perp_z = _perpendicular_vector(angle)

        top_tip = Vector3(
            top_base.x + dir_x * arm_extension,
            ring_height * 0.68,
            top_base.z + dir_z * arm_extension,
        )
        bottom_tip = Vector3(
            bottom_base.x + dir_x * arm_lower_extension,
            -ring_height * 0.68,
            bottom_base.z + dir_z * arm_lower_extension,
        )

        tip_upper_left = Vector3(
            top_tip.x + perp_x * arm_width,
            top_tip.y + ring_height * 0.12,
            top_tip.z + perp_z * arm_width,
        )
        tip_upper_right = Vector3(
            top_tip.x - perp_x * arm_width,
            top_tip.y + ring_height * 0.12,
            top_tip.z - perp_z * arm_width,
        )
        tip_lower_left = Vector3(
            bottom_tip.x + perp_x * arm_width * 0.85,
            bottom_tip.y - ring_height * 0.08,
            bottom_tip.z + perp_z * arm_width * 0.85,
        )
        tip_lower_right = Vector3(
            bottom_tip.x - perp_x * arm_width * 0.85,
            bottom_tip.y - ring_height * 0.08,
            bottom_tip.z - perp_z * arm_width * 0.85,
        )
        ...
        segments.extend(
            [
                ...
                (top_base, middle_base),
                (bottom_base, middle_base),
                (top_base, top_tip),
                (bottom_base, bottom_tip),
                (top_tip, bottom_tip),
                (top_tip, tip_upper_left),
                (top_tip, tip_upper_right),
                (bottom_tip, tip_lower_left),
                (bottom_tip, tip_lower_right),
                (tip_upper_left, tip_lower_left),
                (tip_upper_right, tip_lower_right),
                (tip_upper_left, tip_upper_right),
                (tip_lower_left, tip_lower_right),
            ]
        )

        dorsal_anchor = _nearest_by_z(dorsal_spine_points, top_base.z)
        ventral_anchor = _nearest_by_z(ventral_spine_points, bottom_base.z)
        segments.extend(
            [
                (top_base, dorsal_anchor),
                (bottom_base, ventral_anchor),
                (tip_upper_left, dorsal_anchor),
                (tip_upper_right, dorsal_anchor),
                (tip_lower_left, ventral_anchor),
                (tip_lower_right, ventral_anchor),
            ]
        )
        ...
        segments.extend(
            [
                (inner_top, top_base),
                (inner_bottom, bottom_base),
                (inner_top, top_tip),
                (inner_bottom, bottom_tip),
                (inner_top, middle_base),
                (inner_bottom, middle_base),
            ]
        )
